...
float gaussian_2d(vec2 pos, float coef, float sigma) {
    return coef * exp(-0.5 * (pos.x * pos.x + pos.y * pos.y) / (sigma * sigma));
}

vec2 integrate(vec2 uv) {
    float coef_inner = 1.0 / (2.0 * PI * INNER_RADIUS * INNER_RADIUS);
    float coef_outer = 1.0 / (2.0 * PI * OUTER_RADIUS * OUTER_RADIUS);
    ...
    for (int i = -KERNEL_RADIUS; i <= KERNEL_RADIUS; i++) {
        for (int j = -KERNEL_RADIUS; j <= KERNEL_RADIUS; j++) {
            ...
            vec2 current_uv = fract(uv + pos / iResolution.xy);
            float val = texture(iChannel0, current_uv).r;

            if (d <= (INNER_RADIUS + 0.5)) {
                float w = gaussian_2d(pos, coef_inner, INNER_RADIUS);
                inner_sum += val * w;
                inner_w_sum += w;
            }

            if (d >= (INNER_RADIUS - 0.5) && d <= (OUTER_RADIUS + 0.5)) {
                float w = gaussian_2d(pos, coef_outer, OUTER_RADIUS);
                annulus_sum += val * w;
                annulus_w_sum += w;
            }
        }
